...
async function fetchJsonEA(url: string, log: LogStep[], stepName: string, attemptNumber?: number, totalAttempts?: number): Promise<any> {
  const attemptMessage = totalAttempts && totalAttempts > 1 ? ` (Attempt ${attemptNumber}/${totalAttempts})` : '';
  log.push({ message: `Attempting to fetch ${stepName} from: ${url}${attemptMessage}`, status: 'pending' });
  try {
...
export async function fetchAllUniqueEAParametersAction(
  ...
  try {
    // Fetch a sample of active stations (e.g., first 50 for performance)
    // The EA API can be slow, so limiting samples is important.
    const stationsUrl = `${EA_API_BASE_URL}/id/stations?status=Active&_limit=50`; 
    ...
    log.push({ message: `Found ${stationsData.items.length} active stations in the sample. Fetching measures for up to 30 of them...`, status: 'info' });

    const uniqueParamsSet = new Set<string>();
    let stationsProcessedForMeasures = 0;

    // Iterate through a subset of stations to get their measures
    for (const station of stationsData.items as EAStationInfo[]) {
      ...
      try {
        ...
        if (measuresData && measuresData.items && measuresData.items.length > 0) {
          ...
          (measuresData.items as EAMeasureInfo[]).forEach(measure => {
            if (measure.parameterName) { // parameterName is the human-readable one
      ...
      if (stationsProcessedForMeasures >= 30) { // Limit actual measure fetches to avoid API rate limits/slowness
...
export async function fetchEAStationsForParameterAction(
  ...
  try {
    ...
    const stationsUrl = `${EA_API_BASE_URL}/id/stations?status=Active&parameterName=${encodeURIComponent(selectedParameter)}&_limit=100`; // Limit to 100 stations
    ...
    for (const station of stationsData.items as EAStationInfo[]) {
      ...
      // This ensures we get the correct measure URL ('@id') for time series fetching.
      ...
      try {
        const measuresData = await fetchJsonEA(measuresUrl, log, `specific measures for ${station.label} matching parameter '${selectedParameter}'`, stationsProcessed, stationsData.items.length);
...
export async function fetchEATimeSeriesDataAction(
  ...
  // Note: The EA API can be strict about date ranges and limits. _limit=5000 is a high practical limit.
  const readingsUrl = `${measureId}/readings?_sorted&startdate=${formattedStartDate}&enddate=${formattedEndDate}&_limit=5000`; 
  log.push({ message: `Fetching readings from URL: ${readingsUrl}`, status: 'info' });

  try {
    // First, fetch measure details to get parameter name and unit from the measure object itself
    ...
    try {
        ...
        // The EA API response for a specific measure might or might not be wrapped in 'items'
        // Check if properties exist directly, otherwise try to access from items[0]
        if (measureDetailsData.parameterName && measureDetailsData.unitName) {
            pName = measureDetailsData.parameterName; 
            uName = measureDetailsData.unitName;
            log.push({ message: `Successfully extracted measure details directly: Param='${pName}', Unit='${uName}' from ${measureDetailsUrl}`, status: 'success' });
        } else if (measureDetailsData.items && Array.isArray(measureDetailsData.items) && measureDetailsData.items.length > 0) {
            ...
            log.push({ message: `Could not extract parameterName/unitName from ${measureDetailsUrl}. Response might be unexpected: ${JSON.stringify(measureDetailsData).substring(0,100)}...`, status: 'warning' });
        }
    } catch (e) {
