...
import { Loader2, SunMoon, LayoutGrid, CloudSun, Waves, Search, Info, CheckCircle2, XCircle, ListChecks, FileText, MapPin, CalendarDays, ChevronDown, Timer, Compass, Sailboat } from "lucide-react";
...
// OM Marine Imports
import type { MarineDataPoint as OMMarineDataPoint, LogStep as OMMLogStep, MarinePlotVisibilityKeys } from './om-marine-shared';
...
const omPlotConfigIcons: Record<MarinePlotVisibilityKeys, React.ElementType> = {
  seaLevel: Waves, waveHeight: Sailboat, waveDirection: Compass, wavePeriod: Timer,
};
...
// For EA
const eaKnownStations: { key: string; name: string; id: string; lat?:number; long?:number}[] = [
  { key: "milfordhaven_ea", name: "Milford Haven (EA)", id: "E71524", lat: 51.706, long: -5.036 }, // Example ID, verify actual ID for measures
  { key: "newlyn_ea", name: "Newlyn (EA)", id: "E72534", lat:50.103, long: -5.547 },
  { key: "dover_ea", name: "Dover (EA)", id: "E71901", lat:51.119, long: 1.320 },
  { key: "liverpool_ea", name: "Liverpool (Gladstone Dock) (EA)", id: "E70604", lat: 53.453, long: -3.020},
];
const eaDefaultStationKey = "milfordhaven_ea";
...
export default function OpenAccessDataExplorerPage() {
  ...
  const [eaShowParamFetchLogAccordion, setEaShowParamFetchLogAccordion] = useState<string>("");
  ...
  const [eaShowStationFetchLogAccordion, setEaShowStationFetchLogAccordion] = useState<string>("");
  ...
  const [showEaStationList, setShowEaStationList] = useState(false);
  ...
  const [eaShowTimeSeriesFetchLogAccordion, setEaShowTimeSeriesFetchLogAccordion] = useState<string>("");
  ...
  const handleOMFetchMarineData = async (coords: { latitude: number; longitude: number }, locationName: string) => {
    ...
    setOmFetchLogSteps([]); setOmIsLogLoading(true); setOmLogOverallStatus('pending'); setOmShowFetchLogAccordion("om-fetch-log-item");
    ...
    if (result.success && result.data) {
      ...
      if (result.data.length === 0 && !result.error) { 
        toast({ variant: "default", title: "No Data", description: "No Open-Meteo Marine data points found for the selected criteria.", duration: 4000 });
        setOmLogOverallStatus('warning');
  ...
  useEffect(() => {
    ...
    setOmShowSuggestions(filtered.length > 0 && isFocused);
  }, [omSearchTerm]);
  ...
  const handleLoadEAParameters = async () => {
    ...
    setEaParamFetchLogSteps([]); setEaIsParamLogLoading(true); setEaParamLogStatus('pending'); setEaShowParamFetchLogAccordion("ea-param-log-item");
  ...
  const handleEASelectParameter = async (parameter: string | null) => {
    ...
    setEaStationsForParameter(null); setEaErrorStations(null); 
    setEaSelectedStationDetails(null); setShowEaStationList(false);
    setEaTimeSeriesData(null); 

    if (parameter) {
      setEaIsLoadingStations(true);
      setEaStationFetchLogSteps([]); setEaIsStationLogLoading(true); setEaStationLogStatus('pending'); setEaShowStationFetchLogAccordion("ea-station-log-item");
      ...
      if (result.success && result.stations) {
        ...
        setShowEaStationList(true); // Show station list after fetching
        if (result.stations.length === 0) {
          toast({title: "No Stations", description: `No EA stations found measuring '${parameter}'.`});
          setEaStationLogStatus('warning'); // Keep log open if no stations
        } else {
          toast({title: "Stations Loaded", description: `Found ${result.stations.length} stations for '${parameter}'.`});
          setEaStationLogStatus('success'); setEaShowStationFetchLogAccordion("");
        }
      } else {
  ...
  const handleEAFetchTimeSeries = async () => {
    ...
    setEaTimeSeriesFetchLogSteps([]); setEaIsTimeSeriesLogLoading(true); setEaTimeSeriesLogStatus('pending'); setEaShowTimeSeriesFetchLogAccordion("ea-timeseries-log-item");
    ...
    if (result.success && result.data) {
      ...
      if(result.data.length === 0) {
         toast({title: "No Time Series Data", description: `No data points found for ${eaSelectedStationDetails.label} in the selected period.`});
         setEaTimeSeriesLogStatus('warning');
      } else {
         toast({title: "Data Plotted", description: `Displaying ${result.data.length} points for ${eaSelectedStationDetails.label}.`});
         setEaTimeSeriesLogStatus('success'); setEaShowTimeSeriesFetchLogAccordion("");
      }
    } else {
          ...
          <CardHeader className="pb-3 pt-4">
            <CardTitle className="text-lg flex items-center gap-2">
             ...
             <CardDescription className="text-xs">
                Select a data source to explore. Environment Agency (EA) data provides UK river levels, rainfall, groundwater, etc. Open-Meteo Marine offers global sea level and wave data.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col sm:flex-row gap-2">
            <Button onClick={() => { setCurrentMode('ea'); setShowEaStationList(false); setEaSelectedStationDetails(null); setEaTimeSeriesData(null); setEaSelectedParameter(null); setEaStationsForParameter(null); setEaUniqueParameters(null);}} 
            ...
            <Button onClick={() => { setCurrentMode('om-marine'); const df = omKnownLocations[omDefaultLocationKey]; if(df) { setOmSearchTerm(df.name); setOmInitialCoords({latitude: df.lat, longitude: df.lon}); setOmCurrentLocationName(df.name); } else {setOmSearchTerm(""); setOmInitialCoords(null); setOmCurrentLocationName(null);}}} 
        ...
        {currentMode === 'ea' && (
          <div className="space-y-3">
            ...
            <Card>
              ...
              {renderLogAccordion(eaParamFetchLogSteps, eaShowParamFetchLogAccordion, setEaShowParamFetchLogAccordion, eaIsParamLogLoading, eaParamLogStatus, "Parameter Discovery Log", eaErrorParameters)}
            </Card>

            {/* Card 2: Select Station */}
            {eaSelectedParameter && (showEaStationList || (eaStationsForParameter && eaStationsForParameter.length > 0 && !eaSelectedStationDetails)) && (
            <Card>
                <CardHeader className="pb-2 pt-3">
                    <CardTitle className="text-base">2. Select Station for: <span className="font-semibold text-primary">{eaSelectedParameter}</span></CardTitle>
                     <Button variant="link" size="sm" className="text-xs h-auto p-0 mt-1" onClick={() => { setEaSelectedParameter(null); setEaStationsForParameter(null); setEaSelectedStationDetails(null); setEaTimeSeriesData(null); setShowEaStationList(false);}}>
                        Change Parameter
                    </Button>
                </CardHeader>
                <CardContent>
                 ...
                 {renderLogAccordion(eaStationFetchLogSteps, eaShowStationFetchLogAccordion, setEaShowStationFetchLogAccordion, eaIsStationLogLoading, eaStationLogStatus, "Station Filter Log", eaErrorStations)}
            </Card>
            )}
            
            {/* Card 3: Plotting Area */}
            ...
            <Card>
                <CardHeader className="pb-2 pt-3">
                    ...
                    <div className="flex gap-2">
                        <Button variant="link" size="sm" className="text-xs h-auto p-0 mt-1" onClick={() => { setEaSelectedStationDetails(null); setEaTimeSeriesData(null); setShowEaStationList(true); /* Re-show station list */ }}>
                            Change Station
                        </Button>
                         <Button variant="link" size="sm" className="text-xs h-auto p-0 mt-1" onClick={() => { setEaSelectedParameter(null); setEaStationsForParameter(null); setEaSelectedStationDetails(null); setEaTimeSeriesData(null); setShowEaStationList(false);}}>
                            Change Parameter
                ...
                <CardContent className="space-y-3">
                 ...
                 {renderLogAccordion(eaTimeSeriesFetchLogSteps, eaShowTimeSeriesFetchLogAccordion, setEaShowTimeSeriesFetchLogAccordion, eaIsTimeSeriesLogLoading, eaTimeSeriesLogStatus, "Time Series Fetch Log", eaErrorTimeSeries)}
            </Card>
            )}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                ...
                <CardContent className="space-y-2">
                  <div className="relative">
                    <Input type="text" placeholder="Search UK coastal location..." value={omSearchTerm}
                           ...
                           onBlur={() => setTimeout(() => setOmShowSuggestions(false), 150)} // Delay to allow click on suggestion
                    ...
                    {omShowSuggestions && omSuggestions.length > 0 && (
                      <div className="absolute z-20 w-full mt-0 bg-card border rounded-md shadow-lg max-h-60 overflow-y-auto">
                        {omSuggestions.map((s) => <button key={s.key} type="button" className="w-full text-left px-3 py-1.5 text-xs hover:bg-muted focus:bg-muted focus:outline-none" onClick={() => handleOMSuggestionClick(s.key)} onMouseDown={(e) => e.preventDefault() /* Prevents blur before click */}>{s.name}</button>)}
                      </div>
                    )}
                  </div>
                  {omInitialCoords && <p className="text-xs text-muted-foreground text-center">Lat: {omInitialCoords.latitude.toFixed(3)}, Lon: {omInitialCoords.longitude.toFixed(3)}</p>}
                  <div>
            ...
            <div className="md:col-span-8 lg:col-span-9">
              <Card className="shadow-sm h-full">
                ...
                <CardContent className="p-1.5 h-[calc(100%-2.5rem)]"> {/* Ensure sufficient height */}
      ...
      <footer className="py-3 md:px-4 md:py-0 border-t">
        <div className="container flex flex-col items-center justify-center gap-2 md:h-12 md:flex-row">
          <p className="text-balance text-center text-xs leading-loose text-muted-foreground">
            Marine data from Open-Meteo. EA data from Environment Agency. CSV Explorer. Weather Page.
          </p>
        </div>
      </footer>
    </div>
