...
import {
  ...
  Move as MoveIcon, ArrowUpRight, Spline
} from "lucide-react";
...
const DEFAULT_PLOT_HEIGHT = 272; // Base height for chart rendering area
const EXPANDED_PLOT_HEIGHT = 544; // Expanded height
...
export function PlotInstance({ instanceId, onRemovePlot, initialPlotTitle = "Data Plot" }: PlotInstanceProps) {
  ...
  const CHART_RENDERING_BASE_HEIGHT = useMemo(() => DEFAULT_PLOT_HEIGHT, []); // Or currentChartHeight if clipping needs to adapt
  ...
  const isContextualToolbarDisabled = useMemo(() => isDraggingToolbar || !!draggingPoint || !!movingLineId, [isDraggingToolbar, draggingPoint, movingLineId]);
  ...
  const parseAndValidateCsv = useCallback((csvText: string, fileName: string): { data: DataPoint[], seriesNames: string[], timeHeader: string } | null => {
    ...
    if (localLines.length < 2) {
      updateStepStatus('headerParse', 'error', `CSV error: Must have a header row and at least one data row.`);
      const headerParseIndex = initialValidationSteps.findIndex(s => s.id === 'headerParse');
      initialValidationSteps.slice(headerParseIndex + 1).forEach(s => updateStepStatus(s.id, 'error', 'Prerequisite step failed.'));
      return null;
    }
    ...
    if (potentialVariableHeaders.length > 0 && potentialVariableHeaders[potentialVariableHeaders.length - 1].trim().toLowerCase() === 'rec') {
      ...
      updateStepStatus('variableColumnCheck', 'success', `Identified variable columns. Last column "Rec" excluded.`);
    } else if (potentialVariableHeaders.length === 0 && originalHeaders.length > 1) {
       updateStepStatus('variableColumnCheck', 'error', `No data variable columns found after the first (time) column. Delimiters: comma, semicolon, or tab.`);
       ...
       return null;
        ...
        updateStepStatus('variableColumnCheck', 'error', `Expected at least two columns (time + one variable). Found ${originalHeaders.length}. Delimiters: comma, semicolon, or tab.`);
    ...
    if (actualVariableHeadersToProcess.length === 0) {
        const yAxisErrorMsg = (originalHeaders.length === 2 && originalHeaders[1].trim().toLowerCase() === 'rec') ?
            ...
            `No plottable variable columns found after processing headers.`;
    ...
    if (uniqueSeriesNamesForDropdown.length > 0) {
       ...
       updateStepStatus('yAxisFirstVarIdentified', 'success', `Col 2 ('${firstVarOriginalHeader}') is 1st var (key: '${firstVarPlotKey}'). Total: ${uniqueSeriesNamesForDropdown.length}.`);
    } else {
      updateStepStatus('yAxisFirstVarIdentified', 'error', `No plottable variable columns identified. Check CSV structure and delimiters (comma, semicolon, or tab).`);
      return null;
    }
    ...
    if (data.length === 0) {
      updateStepStatus('dataRowFormat', 'error', `No processable data rows found. Ensure variable columns contain numeric data. Delimiters: comma, semicolon, or tab.`);
      return null;
    }
  ...
  }, [updateStepStatus, setValidationSteps]);
  ...
  const handleFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      updateAndReturnError('fileType', `File name does not end with .csv. Please select a valid CSV file.`, "Unsupported File Type");
      setIsProcessing(false); if (event.target) event.target.value = ""; return;
    }
    ...
    try {
      ...
      if (!fileContent.trim()) {
        updateAndReturnError('fileRead', `File is empty or contains only whitespace.`, "Empty File");
        setIsProcessing(false); if (event.target) event.target.value = ""; return;
      }
      updateStepStatus('fileRead', 'success', 'File content read successfully.');
    } catch (e: any) {
      const errorMsg = e instanceof Error ? e.message : String(e);
      updateAndReturnError('fileRead', `Could not read content: ${errorMsg}. It may be corrupted.`, "File Read Error");
      setIsProcessing(false); if (event.target) event.target.value = ""; return;
    }
    
    processCsvFileContent(fileContent, file.name);
    setIsProcessing(false); if (event.target) event.target.value = "";
  }, [instanceId, processCsvFileContent, toast, updateStepStatus, setValidationSteps]);
  ...
  const handleLoadSavedPlotFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (successfullyProcessed && actualSeriesInLoadedCsv) {
        ...
        setBrushStartIndex(savedState.brushStartIndex); setBrushEndIndex(savedState.brushEndIndex); setLines(savedState.lines || []); 
    ...
    setIsProcessing(false); if (event.target) event.target.value = ""; 
  }, [instanceId, processCsvFileContent, toast, setValidationSteps]);
  ...
  const handleSavePlot = useCallback(() => {
    ...
    const stateToSave: SavedPlotState = {
      ...
      isPlotExpanded, isMinimalistView, brushStartIndex, brushEndIndex, lines, 
    };
  ...
  }, [rawCsvText, currentFileName, plotTitle, timeAxisLabel, dataSeries, visibleSeries, isPlotExpanded, isMinimalistView, brushStartIndex, brushEndIndex, lines, toast]);
  ...
  const updateContextualToolbarPos = useCallback((line: LineAnnotation | null) => {
    ...
    if (toolbarY - (TOOLBAR_APPROX_HEIGHT / 2) < VERTICAL_GAP_TOOLBAR) toolbarY = midY + TOOLBAR_APPROX_HEIGHT / 2 + TOOLBAR_OFFSET_FROM_LINE_Y;
  ...
  const handleAddLine = useCallback(() => {
    ...
    const newLine: LineAnnotation = {
      id: Date.now().toString() + instanceId, x1: centerX - defaultLineLength / 2, y1: centerY,
    ...
    setLines(prevLines => [...prevLines, newLine]); setSelectedLineId(newLine.id); updateContextualToolbarPos(newLine);
  ...
  const handleDraggablePointInteractionStart = useCallback((lineId: string, pointType: 'start' | 'end', event: React.MouseEvent | React.TouchEvent) => {
    event.stopPropagation(); if ('preventDefault' in event && event.type.startsWith('touch')) event.preventDefault(); 
    setSelectedLineId(lineId); setDraggingPoint({ lineId, pointType });
  }, []);

  const handleLineHitboxInteractionStart = useCallback((lineId: string, event: React.MouseEvent | React.TouchEvent<Element>) => {
    event.stopPropagation(); if (anyAnnotationInteractionActive && !movingLineId) return;
    ...
    setMovingLineId(lineId); setDragStartCoords({ x, y }); setLineBeingMovedOriginalState({ ...lineToInteract });
    updateContextualToolbarPos(lineToInteract);
  }, [lines, anyAnnotationInteractionActive, movingLineId, getNormalizedCoordinates, updateContextualToolbarPos]);

  const handleToolbarDragStart = useCallback((event: React.MouseEvent | React.TouchEvent) => {
    event.stopPropagation(); if (!contextualToolbarPosition) return; 
    setIsDraggingToolbar(true); const { clientX, clientY } = getNormalizedCoordinates(event);
    setToolbarDragStart({ x: clientX, y: clientY }); setToolbarInitialPosition({ ...contextualToolbarPosition }); 
    ...
    if (draggingPoint) {
      ...
      setLines(prevLines => prevLines.map(line => {
        if (line.id === movingLineId) {
          const updatedLine = {
            ...line, x1: Math.max(0, Math.min(lineBeingMovedOriginalState.x1 + dx, rect.width)),
      ...
      const dxGlobal = clientX - toolbarDragStart.x; const dyGlobal = clientY - toolbarDragStart.y;
      let newToolbarX = toolbarInitialPosition.x + dxGlobal; let newToolbarY = toolbarInitialPosition.y + dyGlobal;
  ...
  }, [draggingPoint, movingLineId, dragStartCoords, lineBeingMovedOriginalState, getNormalizedCoordinates, updateContextualToolbarPos, isDraggingToolbar, toolbarDragStart, toolbarInitialPosition, lines]); // Added lines to dep array
  ...
  const handleInteractionEnd = useCallback(() => {
    ...
    setDraggingPoint(null); setMovingLineId(null); setDragStartCoords(null); setLineBeingMovedOriginalState(null);
    if (isDraggingToolbar) { setIsDraggingToolbar(false); setToolbarDragStart(null); setToolbarInitialPosition(null); }
  ...
  const handleSvgBackgroundClick = useCallback((event: React.MouseEvent<SVGSVGElement> | React.TouchEvent<SVGSVGElement>) => {
    ...
    if (targetIsSvg && !anyAnnotationInteractionActive) { setSelectedLineId(null); setContextualToolbarPosition(null); }
  ...
  }, [selectedLineId, anyAnnotationInteractionActive, lines]); // Added lines
  ...
  const handleDeleteSelectedLine = useCallback(() => {
    if (selectedLineId && !anyAnnotationInteractionActive) {
      ...
      setSelectedLineId(null); setContextualToolbarPosition(null);
    }
  }, [selectedLineId, anyAnnotationInteractionActive, lines]); // Added lines
  ...
  const handleCopySelectedLine = useCallback(() => {
    if (selectedLineId && !anyAnnotationInteractionActive) {
      ...
      if (lineToCopy && svgOverlayRef.current) {
        const svgRect = svgOverlayRef.current.getBoundingClientRect();
        let offsetX = Math.min(10, svgRect.width * 0.02); let offsetY = Math.min(10, svgRect.height * 0.02);
        ...
        const newLine: LineAnnotation = {
          ...lineToCopy, id: Date.now().toString() + instanceId, 
          x1: Math.max(0, Math.min(newX1, svgRect.width)), y1: Math.max(0, Math.min(newY1, svgRect.height)),
          x2: Math.max(0, Math.min(newX2, svgRect.width)), y2: Math.max(0, Math.min(newY2, svgRect.height)),
        };
        setLines(prevLines => [...prevLines, newLine]); setSelectedLineId(newLine.id); updateContextualToolbarPos(newLine);
      }
    }
  ...
  const isMainToolbarButtonDisabled = useCallback((buttonType?: 'style' | 'color' | 'action') => {
    if (anyAnnotationInteractionActive) return true;
    if (buttonType === 'style' || buttonType === 'color') return !selectedLineId;
    return false; 
  ...
  }, [isProcessing, validationSteps, instanceId]); // Removed accordionValue from dep array as it's set here
  ...
  // --- JSX ---
  const summaryStep = getSummaryStep(); // This uses useCallback, so it's stable unless its own deps change
  const csvFileInputId = `${uniqueComponentId}-csv-upload-${instanceId}`;
  const jsonLoadInputId = `${uniqueComponentId}-json-load-${instanceId}`;

  // Conditional rendering logic for UI elements can be complex and might appear to change hook order if not careful
  // But the hooks themselves are called above, unconditionally.
  // The derived values are then used in the JSX.
  ...
  return (
    <Card className="shadow-lg">
      ...
      {!isMinimized && (
        <CardContent className={cn("p-2 pt-1 md:grid", isMinimalistView ? "block" : "md:grid-cols-12 gap-2")}>
          {!isMinimalistView && (
            <>
          ...
          <div className={cn("flex flex-col min-h-0", isMinimalistView ? "col-span-full" : "md:col-span-8")}>
             {isOverlayActive && parsedData.length > 0 && (
              <TooltipProvider delayDuration={100}>
                <div className="bg-card border shadow-lg rounded-md p-1 flex items-center space-x-0.5 mb-1 self-start">
                  <Tooltip><TooltipTrigger asChild><Button variant="outline" className="h-7 px-1.5" onClick={handleAddLine} disabled={isMainToolbarButtonDisabled('action')}><Plus className="h-3.5 w-3.5 mr-1" /> <span className="text-xs">Line</span></Button></TooltipTrigger><TooltipContent><p>Add Line</p></TooltipContent></Tooltip>
                  <Separator orientation="vertical" className="h-5 mx-0.5" />
                  <DropdownMenu><Tooltip><TooltipTrigger asChild><DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="h-7 w-7" disabled={isMainToolbarButtonDisabled('style')} aria-label="Line Style & Thickness Options"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5"><path d="M2 3h12v1.5H2zM2 7.25h12v2.5H2zM2 11.5h12v3.5H2z"/></svg></Button></DropdownMenuTrigger></TooltipTrigger><TooltipContent><p>Line Style & Thickness</p></TooltipContent></Tooltip>
                    <DropdownMenuContent className="w-56"><DropdownMenuLabel>Line Style</DropdownMenuLabel><DropdownMenuSeparatorShadcn /><DropdownMenuRadioGroup value={selectedLineForToolbar?.lineStyle || 'solid'} onValueChange={(value) => handleLineStyleChange(value as 'solid' | 'dashed' | 'dotted')}>{['solid', 'dashed', 'dotted'].map(s => <DropdownMenuRadioItem key={s} value={s} className="text-xs"><LineStyleIcon style={s as 'solid' | 'dashed' | 'dotted'} /></DropdownMenuRadioItem>)}</DropdownMenuRadioGroup><DropdownMenuSeparatorShadcn /><DropdownMenuLabel>Stroke Weight</DropdownMenuLabel><div className="px-2 py-1.5 flex items-center space-x-2"><Slider defaultValue={[DEFAULT_STROKE_WIDTH]} value={[selectedLineForToolbar?.strokeWidth || DEFAULT_STROKE_WIDTH]} onValueChange={handleStrokeWeightChange} min={1} max={10} step={0.5} disabled={isMainToolbarButtonDisabled('style')} className="flex-grow" /><span className="text-xs w-12 text-right">{(selectedLineForToolbar?.strokeWidth || DEFAULT_STROKE_WIDTH).toFixed(1)}px</span></div></DropdownMenuContent>
                  </DropdownMenu>
                  <DropdownMenu><Tooltip><TooltipTrigger asChild><DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="h-7 w-7" disabled={isMainToolbarButtonDisabled('style')} aria-label="Arrow Style Options"><MoveRight className="h-3.5 w-3.5" /></Button></DropdownMenuTrigger></TooltipTrigger><TooltipContent><p>Arrow Style</p></TooltipContent></Tooltip>
                     <DropdownMenuContent className="w-56"><DropdownMenuLabel>Arrow Style</DropdownMenuLabel><DropdownMenuSeparatorShadcn /><DropdownMenuRadioGroup value={selectedLineForToolbar?.arrowStyle || 'none'} onValueChange={(value) => handleArrowStyleChange(value as 'none' | 'end' | 'both')}>{[['none', "No Arrowhead"], ['end', "Arrow at End"], ['both', "Arrows at Both Ends"]].map(([styleVal, label]) => <DropdownMenuRadioItem key={styleVal} value={styleVal} className="text-xs"><ArrowStyleIcon style={styleVal as 'none' | 'end' | 'both'} uniqueId={`${instanceId}-${styleVal}`} />{label}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup></DropdownMenuContent>
                  </DropdownMenu>
                  <DropdownMenu><Tooltip><TooltipTrigger asChild><DropdownMenuTrigger asChild><Button variant="outline" size="icon" className="h-7 w-7" disabled={isMainToolbarButtonDisabled('color')} aria-label="Line Color Options"><Palette className="h-3.5 w-3.5" /></Button></DropdownMenuTrigger></TooltipTrigger><TooltipContent><p>Line Color</p></TooltipContent></Tooltip>
                    <DropdownMenuContent className="w-56"><DropdownMenuLabel>Line Color</DropdownMenuLabel><DropdownMenuSeparatorShadcn />
            ...
            <div ref={chartAreaRef} className={cn("relative flex-1 min-h-0", (isOverlayActive && (anyAnnotationInteractionActive || (selectedLineId && !isDraggingToolbar))) && "opacity-70")} style={{ height: `${CHART_RENDERING_BASE_HEIGHT * 0.85}px` }}> {/* Apply clipping height here */}
                <ChartDisplay
                    data={parsedData}
                ...
                />
                 {isOverlayActive && (
                    <svg
                        ...
                        style={{ cursor: svgCursor, pointerEvents: (isOverlayActive && !anyAnnotationInteractionActive) ? 'auto' : (anyAnnotationInteractionActive ? 'auto' : 'none')}}
                    >
                        <defs>
                        ...
                        {lines.map((line) => (
                        <g key={line.id}>
                            <line // Hitbox for selection and move
                            ...
                            style={{ pointerEvents: (draggingPoint || (movingLineId === line.id && anyAnnotationInteractionActive) || (movingLineId && movingLineId !== line.id) || isDraggingToolbar) ? 'none' : 'auto' }}
                            />
                            <line // Visible line
                            x1={line.x1} y1={line.y1} x2={line.x2} y2={line.y2}
               ...
               {isOverlayActive && selectedLineId && contextualToolbarPosition && (
                  <div
                  ...
                  >
                      <TooltipProvider delayDuration={100}>
                          ...
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleCopySelectedLine} disabled={isContextualToolbarDisabled} aria-label="Copy Selected Line"><Copy className="h-3 w-3" /></Button></TooltipTrigger><TooltipContent side="bottom"><p>Copy Line</p></TooltipContent></Tooltip>
                          <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleDeleteSelectedLine} disabled={isContextualToolbarDisabled} aria-label="Delete Selected Line"><Trash2 className="h-3 w-3" /></Button></TooltipTrigger><TooltipContent side="bottom"><p>Delete Line</p></TooltipContent></Tooltip>
                      </TooltipProvider>
                  </div>
              )}
            </div>
